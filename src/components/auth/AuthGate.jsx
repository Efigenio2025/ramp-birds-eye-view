  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-ramp-bg text-ramp-text">
        <div className="mx-auto max-w-7xl px-4 py-10 text-sm text-ramp-muted">
          Loading…
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen bg-ramp-bg text-ramp-text">
        <div className="mx-auto max-w-7xl px-4 py-10">
          <div className="mb-4">
            <div className="text-2xl font-bold">Ramp Birds Eye View</div>
            <div className="text-sm text-ramp-muted">
              Mobile entry access (magic link)
            </div>
          </div>
          <LoginCard />
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-4">
        <div className="mb-4 flex items-center justify-between rounded-2xl border border-ramp-border bg-ramp-panel p-3">
          <div className="text-xs text-ramp-muted">
            Signed in as <span className="font-semibold text-ramp-text">{session.user.email}</span>
          </div>
          <button
            onClick={signOut}
